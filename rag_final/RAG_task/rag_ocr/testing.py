...
# Function to read PDF files and extract images
    ...
    try:
        ...
        for page in doc:
            ...
            for img_index, img in enumerate(page.get_images(full=True)):
                ...
                images.append((img_index, Image.open(io.BytesIO(image_bytes))))
    ...
    for filename in os.listdir(data_folder_path):
        ...
        try:
            if filename.endswith('.pdf'):
                text, images = read_pdf(file_path)
            ...
            # Handle images extracted from PDF/DOCX
            for img_index, image in enumerate(images):
                if isinstance(image, tuple):  # Check if the image is a tuple
                    image_obj = image[1]  # Get the PIL Image from the tuple
                    img_title = f"{sanitized_title}_img_{img_index}.png"
                    try:
                        # Ensure the image has valid dimensions
                        if image_obj.size[0] > 0 and image_obj.size[1] > 0:
                            image_obj.save(os.path.join(data_folder_path, img_title))  # Save image to folder
                            logging.info(f"Extracted and saved image: {img_title}")
                        else:
                            logging.warning(f"Invalid image dimensions for {img_title}. Skipping save.")
                    except Exception as e:
