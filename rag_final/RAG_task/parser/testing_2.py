...
def format_docs(docs):
    ...
    for doc in docs:
        if isinstance(doc, dict) and 'content' in doc and 'id' in doc:
            ...
            file_name = "FinancialData.pdf"  # Explicitly set the correct file name
...
def perform_search(user_question):
    ...
    if answer:
        ...
        source_link = f"Source: [Open Document ID: FinancialData.pdf](file:///{urllib.parse.quote('D:/Genai_project/Retrieval Augmented Generation/rag_final/RAG_task/data_files/FinancialData.pdf')})"
        full_answer = f"{answer}\n\n{source_link}"
        logging.info(f"Full Answer: {full_answer}")
        return full_answer
    else:
...
#             # Construct the file path using forward slashes
#             file_name = doc['id']
#             # Replace backslashes with forward slashes before using it in the f-string
#             file_path = f"D:/Genai_project/Retrieval Augmented Generation/rag_final/RAG_task/data_files/{file_name}".replace("\\", "/")
#             file_link = f"file:///{urllib.parse.quote(file_path)}"  # URL encode the path
#             doc_link = f"[Open Document ID: {file_name}]({file_link})"
#             formatted_sources.append(f"Source: {doc_link}")
...
#     if answer:
#         # Create the final answer with the answer, thanks message, and the source link
#         final_output = f"{answer}\n\nThanks for asking!\n\n{formatted_sources}"
#         logging.info(f"Full Answer: {final_output}")
#         return final_output
#     else:
#         logging.warning("No answer generated by the model.")
#         return None
